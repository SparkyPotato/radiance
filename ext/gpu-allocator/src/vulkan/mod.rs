...
/// Doing this data transfer correctly without invoking undefined behavior can be quite fraught and
...
impl Allocation {
	...
	/// directly. However, if `self` is not actually a valid [`Slab`] (this function would return `None` as described
	/// above), then trying to use it as a [`Slab`] will panic.
	///
	/// # Safety
	///
	...
	pub fn chunk_id(&self) -> Option<std::num::NonZeroU64> { self.chunk_id }

	/// Returns the [`vk::MemoryPropertyFlags`] of this allocation.
	pub fn memory_properties(&self) -> vk::MemoryPropertyFlags { self.memory_properties }
	...
	pub unsafe fn memory(&self) -> vk::DeviceMemory { self.device_memory }

	/// Returns [`true`] if this allocation is using a dedicated underlying allocation.
	pub fn is_dedicated(&self) -> bool { self.dedicated_allocation }
	...
	pub fn mapped_ptr(&self) -> Option<std::ptr::NonNull<std::ffi::c_void>> { self.mapped_ptr.map(|SendSyncPtr(p)| p) }
	...
	pub fn mapped_slice(&self) -> Option<&[u8]> {
		self.mapped_ptr()
			.map(|ptr| unsafe { std::slice::from_raw_parts(ptr.cast().as_ptr(), self.size as usize) })
	}
	...
	pub fn mapped_slice_mut(&mut self) -> Option<&mut [u8]> {
		self.mapped_ptr()
			.map(|ptr| unsafe { std::slice::from_raw_parts_mut(ptr.cast().as_ptr(), self.size as usize) })
	}

	pub fn is_null(&self) -> bool { self.chunk_id.is_none() }
}
...
unsafe impl presser::Slab for MappedAllocationSlab<'_> {
	fn base_ptr(&self) -> *const u8 { self.mapped_ptr }

	fn base_ptr_mut(&mut self) -> *mut u8 { self.mapped_ptr }
...
impl MemoryBlock {
	fn new(
		device: &ash::Device, size: u64, mem_type_index: usize, mapped: bool, buffer_device_address: bool,
		allocation_scheme: AllocationScheme, requires_personal_block: bool,
	) -> Result<Self> {
		let device_memory = {
		...
		let mapped_ptr = mapped
			.then(|| {
				unsafe { device.map_memory(device_memory, 0, vk::WHOLE_SIZE, vk::MemoryMapFlags::empty()) }
					.map_err(|e| {
					...
					.and_then(|p| {
						std::ptr::NonNull::new(p)
							.map(SendSyncPtr)
							.ok_or_else(|| AllocationError::FailedToMap("Returned mapped pointer is null".to_owned()))
					})
			})
			.transpose()?;

		let sub_allocator: Box<dyn allocator::SubAllocator> =
			if allocation_scheme != AllocationScheme::GpuAllocatorManaged || requires_personal_block {
...
impl MemoryType {
	fn allocate(
		&mut self, device: &ash::Device, desc: &AllocationCreateDesc<'_>, granularity: u64, backtrace: Arc<Backtrace>,
		...
		let is_host = self.memory_properties.contains(vk::MemoryPropertyFlags::HOST_VISIBLE);
		...
		// Create a dedicated block for large memory allocations or allocations that require dedicated memory
		// allocations.
		...
		let allocation =
			mem_block
				.sub_allocator
				.allocate(size, alignment, allocation_type, granularity, desc.name, backtrace);
		...
		if mem_block.sub_allocator.is_empty() {
			if mem_block.sub_allocator.supports_general_allocations() {
				if self.active_general_blocks > 1 {
					...
					let block = block.ok_or_else(|| AllocationError::Internal("Memory block must be Some.".into()))?;
...
impl fmt::Debug for Allocator {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { self.generate_report().fmt(f) }
}
...
impl Allocator {
	pub fn new(desc: &AllocatorCreateDesc) -> Result<Self> {
		...
		let memory_types = memory_types
			...
			.map(|(i, mem_type)| MemoryType {
				...
				mappable: mem_type.property_flags.contains(vk::MemoryPropertyFlags::HOST_VISIBLE),
				active_general_blocks: 0,
				buffer_device_address: desc.buffer_device_address,
			})
			.collect::<Vec<_>>();

		let physical_device_properties = unsafe { desc.instance.get_physical_device_properties(desc.physical_device) };
	...
	pub fn allocate(&mut self, desc: &AllocationCreateDesc<'_>) -> Result<Allocation> {
		...
		let mem_loc_preferred_bits = match desc.location {
			...
			MemoryLocation::RebarGpu => {
				vk::MemoryPropertyFlags::HOST_VISIBLE
					| vk::MemoryPropertyFlags::HOST_COHERENT
					| vk::MemoryPropertyFlags::DEVICE_LOCAL
			},
			MemoryLocation::Upload => vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT,
		...
		let Some(memory_type_index) = self.find_memorytype_index(&desc.requirements, mem_loc_preferred_bits) else {
	...
	pub fn rename_allocation(&mut self, allocation: &mut Allocation, name: &str) -> Result<()> {
		...
		mem_block.sub_allocator.rename_allocation(allocation.chunk_id, name)?;
