...
//!
//! `gpu-allocator` recently migrated from [winapi] to [windows-rs] but still provides convenient helpers to convert to
//! and from [winapi] types, enabled when compiling with the `public-winapi` crate feature.
//!
...
//! use gpu_allocator::{d3d12::*, MemoryLocation};
...
//! # #[cfg(feature = "metal")]
//! # fn main() {
//! use gpu_allocator::{metal::*, MemoryLocation};
//! # let device = objc2_metal::MTLCreateSystemDefaultDevice().expect("No MTLDevice found");
//! # let mut allocator = Allocator::new(&AllocatorCreateDesc {
//! #     device: device.clone(),
//! #     debug_settings: Default::default(),
...
impl AllocationSizes {
	...
	pub fn with_max_device_memblock_size(mut self, size: u64) -> Self {
		self.max_device_memblock_size = Self::adjust_memblock_size(size, "Device").max(self.min_device_memblock_size);
	...
	pub fn with_max_host_memblock_size(mut self, size: u64) -> Self {
		self.max_host_memblock_size = Self::adjust_memblock_size(size, "Host").max(self.min_host_memblock_size);
