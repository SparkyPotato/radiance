...
// reduce_fn_mut is no different than reduce_fn, but it introduces "unnecessary" let bindings (and therefore, debug
// symbols or whatever). It's specifically for when $fn is an FnMut.
...
macro_rules! vec_impl_spatial {
	($Vec:ident) => {
		impl<T> $Vec<T> {
			...
			/// Get a copy of this direction vector such that its length equals 1, and also returns the length of the
			/// original vector.
			#[inline]
...
macro_rules! vec_impl_spatial_2d {
	($Vec:ident) => {
		impl<T> $Vec<T> {
			...
			#[deprecated(
				since = "0.14.0",
				note = "This function is opinionated about the semantics of X,Y and Z axii, and should not be used. \
				        The mapping of axii (X,Y,Z) to perceived directions (e.g right, up, forward) is not universal \
				        at all and varies between libraries, graphics APIs, content creation tools and engines. If \
				        you want such helper functions, you should write these yourself as part of the package you're \
				        working on, according to what you know about YOUR current coordinate space."
			)]
			pub fn left() -> Self
			where
			...
			/// Get the unit vector which has `y` set to 1.
			/// This is not intended for screen-space coordinates (in which case the Y axis is reversed). When in doubt,
			/// just use `unit_y()` instead.
			#[deprecated(
				since = "0.14.0",
				note = "This function is opinionated about the semantics of X,Y and Z axii, and should not be used. \
				        The mapping of axii (X,Y,Z) to perceived directions (e.g right, up, forward) is not universal \
...
macro_rules! vec_impl_pixel_rgb {
	($Vec:ident) => {
			...
			T: ColorComponent + Copy + Clone + Primitive,
		{
			...
			// When I first introduced the optional dependency to the `image` crate, ColorType allowed specifying the
			// bit depth procedurally. Now the bit depths are fixed; the only "really" supported T are now u8 and
			// u16... For now, I choose to still "implement" this trait for other T (such as f32), for convenience and
			// backwards compatibility, but you shouldn't use the COLOR_TYPE in that case. Feel free to open an issue
			// about that. NOTE: this comment is duplicated in vec_impl_pixel_rgba!(), please update both instances if
			// you change one
			...
			fn channels(&self) -> &[Self::Subpixel] { self.as_slice() }

			fn channels_mut(&mut self) -> &mut [Self::Subpixel] { self.as_mut_slice() }
			...
			fn to_rgb(&self) -> image::Rgb<Self::Subpixel> { image::Rgb([self.r, self.g, self.b]) }

			fn to_rgba(&self) -> image::Rgba<Self::Subpixel> { image::Rgba([self.r, self.g, self.b, T::full()]) }

			fn to_bgr(&self) -> image::Bgr<Self::Subpixel> { image::Bgr([self.b, self.g, self.r]) }

			fn to_bgra(&self) -> image::Bgra<Self::Subpixel> { image::Bgra([self.b, self.g, self.r, T::full()]) }
			...
			fn to_luma_alpha(&self) -> LumaA<Self::Subpixel> { LumaA([self.to_luma().0[0], T::full()]) }
			...
			fn invert(&mut self) { *self = self.inverted_rgb(); }
...
macro_rules! vec_impl_pixel_rgba {
	($Vec:ident) => {
			...
			T: ColorComponent + Copy + Clone + Primitive,
		{
			...
			// u16... For now, I choose to still "implement" this trait for other T (such as f32), for convenience and
			// backwards compatibility, but you shouldn't use the COLOR_TYPE in that case. Feel free to open an issue
			// about that. NOTE: this comment is duplicated in vec_impl_pixel_rgb!(), please update both instances if
			// you change one
			const COLOR_TYPE: ColorType = match mem::size_of::<T>() {
			...
			fn to_rgba(&self) -> image::Rgba<Self::Subpixel> { image::Rgba([self.r, self.g, self.b, self.a]) }

			fn to_bgr(&self) -> image::Bgr<Self::Subpixel> { image::Bgr([self.b, self.g, self.r]) }

			fn to_bgra(&self) -> image::Bgra<Self::Subpixel> { image::Bgra([self.b, self.g, self.r, self.a]) }
			...
			fn to_luma_alpha(&self) -> LumaA<Self::Subpixel> { LumaA([self.to_luma().0[0], self.a]) }
...
macro_rules! vec_impl_color_rgba {
	($Vec:ident) => {
		...
		impl<T: ColorComponent> Rgba<T> {
			...
			pub fn new_opaque(r: T, g: T, b: T) -> Self { Self::new(r, g, b, T::full()) }

			/// Creates an RGBA color from RGB elements and zero alpha.
			pub fn new_transparent(r: T, g: T, b: T) -> Self { Self::new(r, g, b, T::zero()) }
...
impl From<usize> for ShuffleMask4 {
	fn from(m: usize) -> Self { Self::new(m, m, m, m) }
}
...
impl From<[usize; 4]> for ShuffleMask4 {
	fn from(m: [usize; 4]) -> Self { Self::new(m[0], m[1], m[2], m[3]) }
}
impl ShuffleMask4 {
...
macro_rules! vec_impl_shuffle_4d {
	($Vec:ident($x:tt $y:tt $z:tt $w:tt)) => {
		...
		impl<T> $Vec<T> {
			...
			/// assert_eq!(
			/// 	Vec4::shuffle_lo_hi(a, b, (0, 1, 2, 3)),
			/// 	Vec4::new(0, 1, 6, 7)
			/// );
			/// assert_eq!(
			/// 	Vec4::shuffle_lo_hi(a, b, (3, 2, 1, 0)),
			/// 	Vec4::new(3, 2, 5, 4)
			/// );
			/// ```
			...
			/// ```
			pub fn interleave_0011(a: Self, b: Self) -> Self { Self::new(a.$x, b.$x, a.$y, b.$y) }
			...
			/// ```
			pub fn interleave_2233(a: Self, b: Self) -> Self { Self::new(a.$z, b.$z, a.$w, b.$w) }
			...
			/// ```
			pub fn shuffle_lo_hi_0101(a: Self, b: Self) -> Self { Self::new(a.$x, a.$y, b.$x, b.$y) }
			...
			/// ```
			pub fn shuffle_hi_lo_2323(a: Self, b: Self) -> Self { Self::new(b.$z, b.$w, a.$z, a.$w) }
...
macro_rules! vec_impl_mat2_via_vec4 {
	($Vec:ident) => {
		...
		impl<T: Copy + Add<T, Output = T> + Mul<T, Output = T> + Sub<T, Output = T>> $Vec<T> {
			...
			/// ```
			/// # use vek::Vec4;
			/// let a = Vec4::new(0, 2, 1, 3);
			/// let b = Vec4::new(2, 6, 3, 11);
...
macro_rules! vec_impl_all_vecs {
    ($c_or_simd:ident #[$repr_for_power_of_two_length:meta] $c_or_simd_non_power_of_two:ident #[$repr_for_non_power_of_two_length:meta] $repr_c_non_power_of_two:ident) => {
        ...
        pub mod vec2 {
            ...
            /// Vector type suited for 2D spatial coordinates.
            #[allow(missing_docs)]
            #[derive(Debug, Default, Clone, Copy, Hash, Eq, PartialEq, bevy_reflect::Reflect/*, Ord, PartialOrd*/)]
            #[cfg_attr(feature="serde", derive(Serialize, Deserialize))]
...
mod tests {
	...
	macro_rules! test_vec_t {
		...
		(repr_simd_padding $Vec:ident < $T:ident >) => {
			// This test aims to prove that types in the `repr_simd` modules have no padding/uninit bytes, which is a
			// requirement for implementing bytemuck's Pod trait. In the past, `#[repr(simd)] Vec3<T>` had that issue as
			// the compiler treated it as an aligned Vec4 in memory, but since some time, the compiler stopped
			// supporting non-power-of-two SIMD types, so we were forced to write them as `#[repr(C)]`, which means that
			// now they don't have any padding, but they're also not true SIMD types, which is fine : users should use
			// Vec4 instead if they want SIMD.
			#[test]
