...
macro_rules! quaternion_complete_mod {
	($mod:ident #[$attrs:meta]) => {
		...
		#[derive(Debug, Clone, Copy, Hash, Eq, PartialEq, bevy_reflect::Reflect /* Ord, PartialOrd */)]
		...
		impl<T> Quaternion<T> {
			...
			///
			/// **You are responsible for ensuring that the resulting quaternion is normalized.**
			pub fn from_xyzw(x: T, y: T, z: T, w: T) -> Self { Self { x, y, z, w } }
			...
			T: Copy + Mul<Output = T> + Sub<Output = T> + Zero + Add<T, Output = T>,
		{
			...
			fn mul(self, rhs: Self) -> Self::Output {
				let ((ps, pv), (qs, qv)) = (self.into_scalar_and_vec3(), rhs.into_scalar_and_vec3());
...
pub mod repr_simd {
	...
	use super::{
		super::vec::repr_c::{Vec3 as CVec3, Vec4 as CVec4},
		*,
	};
