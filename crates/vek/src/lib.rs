...
//! # Cargo features
//!
//! - `vec8`, `vec16`, `vec32`, `vec64`, `rgba`, `rgb`, `uvw`, `uv` Enable these types. Other types are always enabled
//!   for the sake of doc-tests.
//! - `repr_simd` enables Nightly Rust's `repr_simd` and `simd_ffi` features, and unlock SIMD versions of all
//!   appropriate types (though `repr_simd` modules). On Stable, this feature has no effect.
...
#![cfg_attr(all(nightly, feature = "platform_intrinsics"), allow(internal_features))]
// We're not scared of core_intrinsics aye... They keep moving around but have actually been there since the dawn of
// time and not changed much.
#![cfg_attr(
